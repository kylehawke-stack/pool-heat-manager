...
const cases: Array<{
...
}> = [
  {
    ...
    input: { declined: false, alreadyAsked: true, askedAt: Date.parse('2026-09-08T22:19:00Z'), latestGuestMs: Date.parse('2026-09-09T13:26:47Z'), now: NOW },
    expect: 'remind',
  },
  {
    name: 'first ambiguous conversation — never asked',
    input: { declined: false, alreadyAsked: false, askedAt: 0, latestGuestMs: NOW - 5 * 60 * 1000, now: NOW },
    expect: 'send',
  },
  {
    name: 'pending, guest silent since the ask — no repeat email every scan',
    input: { declined: false, alreadyAsked: true, askedAt: NOW - 6 * H, latestGuestMs: NOW - 7 * H, now: NOW },
    expect: 'skip-nothing-new',
  },
  {
    name: 'guest message exactly at the ask time is not new',
    input: { declined: false, alreadyAsked: true, askedAt: NOW - 3 * H, latestGuestMs: NOW - 3 * H, now: NOW },
    expect: 'skip-nothing-new',
  },
  {
    name: 'chatty guest — second message inside the hour is held',
    input: { declined: false, alreadyAsked: true, askedAt: NOW - 10 * 60 * 1000, latestGuestMs: NOW - 60 * 1000, now: NOW },
    expect: 'skip-cooldown',
  },
  {
    name: 'cooldown just expired',
    input: { declined: false, alreadyAsked: true, askedAt: NOW - RENUDGE_COOLDOWN_MS, latestGuestMs: NOW - 30 * 60 * 1000, now: NOW },
    expect: 'remind',
  },
  {
    name: 'declined booking is never re-raised, however loudly the guest asks',
    input: { declined: true, alreadyAsked: true, askedAt: NOW - 48 * H, latestGuestMs: NOW - 60 * 1000, now: NOW },
    expect: 'skip-declined',
  },
  {
    name: 'declined wins even on a first ask',
    input: { declined: true, alreadyAsked: false, askedAt: 0, latestGuestMs: NOW, now: NOW },
    expect: 'skip-declined',
  },
  {
    name: 'host-only activity (no guest message) does not nudge',
    input: { declined: false, alreadyAsked: true, askedAt: NOW - 5 * H, latestGuestMs: null, now: NOW },
    expect: 'skip-nothing-new',
  },
  {
    name: 'backlog with no recorded ask time gets one reminder',
    input: { declined: false, alreadyAsked: true, askedAt: 0, latestGuestMs: Date.parse('2026-09-02T12:00:00Z'), now: NOW },
    expect: 'remind',
  },
];
